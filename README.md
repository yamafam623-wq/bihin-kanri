...
8. `/employees`・`/items` にログインせずアクセスすると `/login` にリダイレクトされることを確認する。ログイン後、社員の追加・編集・無効化、備品の追加・編集・発注点変更・在庫数の手動調整（棚卸し）ができることを確認する。
...
tests/
  stock.test.ts               在庫減算・通知ロジックの自動テスト
```
